...
import seispolarity
from .base import WaveformDataset, logger
...
class BohemiaSaxony(WaveformDataset):
    """
    ...
    德国/捷克共和国西北波希米亚和萨克森地区的波形数据和元数据的区域基准数据集。
    ...
    .. warning ::
        ...
        该数据集包含来自西波希米亚当地地震台网 (WEBNET) 的受限数据。
        要编译完整的数据集，您需要提供 EIDA 令牌。
        请参阅 `WEBNET 网站 <https://doi.org/10.7914/SN/WB>`_ 了解更多信息。
...
"""
    ...
    def _download_catalog_colm(self, path: Path = Path.cwd()) -> Path:
        ...
        with NamedTemporaryFile(suffix=".zip", delete=False, dir=path) as temp_file:
            catalog_url = urljoin(seispolarity.remote_root, "auxiliary/collm-catalog.zip")
            download_http(
                catalog_url,
                temp_file.name,
    ...
    async def get_station_waveform_data(
        ...
        """
        Retrieves waveform data for a specific station and event.
        检索特定台站和事件的波形数据。
        """

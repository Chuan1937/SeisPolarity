...
SeisPolarity 提供统一的训练接口，支持检查点保存、早停机制和灵活的配置。
...
config = TrainingConfig(
    ...
    # 日志
    log_every=100,              # 每 N 个 batch 记录一次日志
...
1. **数据加载**：使用 `num_workers > 0` 和 `pin_memory=True` 加速数据加载
2. **批大小**：从小批量（256-512）开始，内存允许时再增大
3. **学习率**：微调时使用较小的学习率（1e-4 到 1e-5）
4. **验证**：始终使用验证集监控过拟合
5. **检查点**：定期保存检查点以避免进度丢失
...
详细的 API 文档请参见 [API 参考](../api/training.md)。

...
SeisPolarity 提供一系列用于地震极性分类的深度学习模型。所有模型均使用 PyTorch 实现，并支持 GPU 加速。
...
SeisPolarity 中的所有模型都提供统一的 PyTorch 接口：
...
残差极性网络（Residual Polarity Network）。
...
自适应极性预测器（Adaptive Polarity Predictor）。
...
# 单样本
waveform = np.random.randn(400)  # 单条波形
waveform = torch.FloatTensor(waveform).unsqueeze(0).unsqueeze(0)  # 添加 batch 和 channel 维度
...
详细的 API 文档请参见 [API 参考](../api/models.md)。
